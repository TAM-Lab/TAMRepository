...
def substitution_ranking(source_word, source_context, candidate_words, fasttext_dico, fasttext_emb, word_count,
                         tokenizer, maskedLM, lables):
...
def main():
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for i in range(eval_size):
            ...
            candidate_words = substitution_generation(mask_words[i], pre_tokens, predicted_top[0].cpu().numpy(), ps,
                                                      num_selection)

            SS.append(candidate_words)

            pre_word = substitution_ranking(mask_words[i], mask_context, candidate_words, fasttext_dico, fasttext_emb,
                                            word_count, tokenizer, model, mask_labels[i])
