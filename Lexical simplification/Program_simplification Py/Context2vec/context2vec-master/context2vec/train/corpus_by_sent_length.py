...
if __name__ == '__main__':
    ...
    corpus_file = open("E:\Simplify\context2vec/train.txt", 'r',encoding="utf-8")
    if len(sys.argv) > 2:
    ...
    corpus_dir = "E:\Simplify\context2vec/train.txt"+'.DIR'
    ...
    for line in corpus_file:
        ...
        if wordnum > 1 and wordnum <= max_sent_len:
            ...
            sub_file.write(line)
            sent_counts[num_filename] += 1#统计相同句子长度的个数
            for word in words:
                word_counts[word] += 1#统计数据集中所有单词词频
            print(word_counts)
