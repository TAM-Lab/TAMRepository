...
def convert_sentence_to_token(sentence, seq_length, tokenizer):
    ...
    for i, word in enumerate(nltk_sent):  # i  xuliehao,word duiyingdeci
...
def BERT_candidate_generation(source_word, pre_tokens, pre_scores, ps, num_selection=20):
    cur_tokens = []
...
def LM_score(source_word, source_context, substitution_selection, tokenizer, maskedLM):
    ...
    candidate = {}
    ss = []
    for substitution in substitution_selection:
        ss.append(substitution)
    source_sentence = ''
    ...
    for substibution in substitution_selection:
        ...
        # print(score)
        LM.append(score)
    candidate = dict(zip(ss, LM))
    candidate = sorted(candidate.items(), key=lambda d: d[1], reverse=True)
    return LM, source_loss, candidate
...
def similarity(source_word, substitution_selection, fasttext_dico, fasttext_emb):
    ss = []
    ##ss_score=[]
    sis_scores = []
    count_scores = []
...
def pre_SR(source_word, substitution_selection, fasttext_dico, fasttext_emb, word_count):
    ss = []
    ##ss_score=[]
    sis_scores = []
    count_scores = []
...
def LM_rank(source_word, source_context, substitution_selection, tokenizer, maskedLM):
    ss = []
    candidate = {}
    for sub in substitution_selection:
        ss.append(sub)
    if len(ss) == 0:
        return source_word
    lm_score, source_lm, candidate = LM_score(source_word, source_context, ss, tokenizer, maskedLM)
    print("lm_source,source_lm:")
    print(lm_score, source_lm)
    print(type(lm_score))

    return lm_score, source_lm, candidate


def substitution_ranking(source_word, source_context, substitution_selection, fasttext_dico, fasttext_emb, word_count,
                         ssPPDB, tokenizer, maskedLM, lables):
    ...
    if len(sis_scores) > 0:
        ...
        all_ranks = [bert + sis for bert, sis in zip(bert_rank, sis_rank)]
    else:
        # all_ranks = [bert+count+LM+ppdb  for bert,count,LM,ppdb in zip(bert_rank,count_rank,lm_rank,ppdb_rank)]
        all_ranks = [bert for bert in zip(bert_rank)]
...
def main():
    ...
    path_csv = "/home/jiayin/Bert/BERT-LS-master/datasets/Bert_SG_sem_fre.csv"
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for i in range(eval_size):
            ...
            cgBERT = BERT_candidate_generation(mask_words[i], pre_tokens, predicted_top[0].cpu().numpy(), ps,
                                               args.num_selections)

            print(cgBERT)

            CGBERT.append(cgBERT)
            # candidates = LM_rank(mask_words[i], cgBERT, fasttext_dico, fasttext_emb)
            # CGBERT.append(ss)
            candidates = pre_SR(mask_words[i],cgBERT, fasttext_dico, fasttext_emb,word_count)
            ...
            # pre_word = substitution_ranking(mask_words[i], mask_context, cgBERT, fasttext_dico, fasttext_emb,word_count, tokenizer, model, mask_labels[i])
            result.append((i, eval_examples[i], mask_words[i], candidates))
            # print(result)
        ...
        precision, accuracy, changed_proportion = evaulation_pipeline_scores(substitution_words, mask_words,
                                                                             mask_labels)
