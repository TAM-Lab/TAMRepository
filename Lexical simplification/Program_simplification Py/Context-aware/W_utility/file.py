...
import csv, shutil, os, sys, glob, pickle, string
...
# read a text file, break lines according to skip
# @param skip: character to skip (default ' ')
def read_file_dict_tokenized (filename, skip = ';', logout = True):
	try:
		data = {}
		fid = open (filename, 'r')
		for line in fid:
			...
			if len(items)>1 and items[0] not in data:
...
# write a dictionary to a csv file (first item of each row is the key)
def write_dict_to_csv_str (filename, data, logout = True):
	try:
		f = open(filename, 'wb')
		csv.writer(f).writerows([k+"	" +v] for k, v in data.iteritems())
		f.close()
	...
	if fin.endswith('.txt'):
		...
		for root, dir, files in os.walk(fin):
		   for filename in files:
		     ...
		     if filename.endswith('.txt') or fin.find('.') ==-1:
