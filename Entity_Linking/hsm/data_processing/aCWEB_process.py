...
def add_unk_gold_id_in_enwiki_and_kb_id():
    ...
    wiki_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-clueweb.csv"
    total_cand_cout = 0
    id_null_count = 0
    ...
    gt_count = 0
    empty_cand = 0
    with open(wiki_train_path, 'r', encoding='utf-8-sig') as csvfile:
        ...
        for row in csv_reader:
            ...
            if "|||".join(row).split("\t")[-3] != "EMPTYCAND":
                ...
                for cand in cand_split:
                    total_cand_cout += 1
                    if len(cand.split("|||")) == 2:
                        print("cand.split==2", row)
                        print("cand.split==2", candidates_cat)
                    if len(cand.split("|||")) > 3:
...
def handle_candidate_excel():
    ...
    clueweb_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-clueweb.csv"
    total_cand_cout = 0
    id_null_count = 0
    men_id_candidates = dict()
    men_name_candidates = dict()
    ...
    duplicate_count = 0
    empty_cand = 0
    with open(clueweb_train_path, 'r', encoding='utf-8-sig') as csvfile:
        csv_reader = csv.reader(csvfile)
        men_count = 0
        for row in csv_reader:
            # i += 1
    ...
    pd.to_pickle(men_id_candidates, "./clueweb/men_id_candidates.pkl")
    # pd.to_pickle(men_name_candidates, './ace2004/men_name_candidates2.pkl')
    pd.to_pickle(men2cand_prior, './clueweb/men2cand_prior.pkl')
    ...
    pd.to_pickle(id2cand, './clueweb/id2cand.pkl')
    ...
    for key ,value in men_id_candidates.items():
        for id in value:
            if id not in id2cand.keys():
                print("id not in id2cand: ", id)
...
def filter_candidates():
    ...
    wiki_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-clueweb.csv"
    men_id_candidates = pd.read_pickle('./clueweb/men_id_candidates.pkl')
    enwiki_id2title = pd.read_pickle("./data/enwiki_id2title2.pkl")
    enwiki_title2id = {v: k for k, v in enwiki_id2title.items()}
    ...
    pd.to_pickle(men_id_candidates_10, './clueweb/men_id_candidates_10.pkl')
...
def construct_aquaint_id_text():
    path = 'D:/DataSet/Entity Linking/WNED/WNED/wned-datasets/clueweb12/RawText'
    listFile = os.listdir(path)
    id_text = dict()
    i = 0
    for file in listFile:
        file_text = ''
        # if file == "Chippenham_United_F.C":
        #    file = "Chippenham_United_F.C."
        # if file == "Alfred_Conkling_Coxe,_Sr":
        #    file = "Alfred_Conkling_Coxe,_Sr."
...
def mapping_xls_xml():
    ...
    aida_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-clueweb.csv"
    gt_count = 0
    with open(aida_train_path, 'r', encoding='utf-8-sig') as csvfile:
        ...
        for row in csv_reader:
            ...
            #    assert 1==2
            text_id = row[0].split("\t")[0].split(" ")[0].strip("'")
            mention = row[0].split("\t")[2]
            # print("|||".join(row).split("\t")[-1].split("|||")[1])
            # if text_id == "William_Wood,_1st_Baron_Hatherley":
            #    print(row)
            if "|||".join(row).split("\t")[-3] != "EMPTYCAND":
                candidates = [row[0].split("\t")[-1]]
                # print(candidates)
    ...
    # print(xls_dict["William_Wood,_1st_Baron_Hatherley"])
    # assert 1==2

    clueweb_kb_xml = 'D:/DataSet/Entity Linking/WNED/WNED/wned-datasets/clueweb12/clueweb.xml'
    kb_file = open(clueweb_kb_xml, 'r', encoding='utf-8')
...
def ace2004_train_data_xls(xls_offset_dict, id_text):
    ...
    enwiki_id2title = pd.read_pickle("./data/enwiki_id2title2.pkl")
    enwiki_title2id = {v: k for k, v in enwiki_id2title.items()}
    wiki_train_data = './clueweb/clueweb_train_xls.txt'
    men_id_candidates_10 = pd.read_pickle('./clueweb/men_id_candidates_10.pkl')
    out_count = 0
    out_count2 = 0
    # print(xls_offset_dict["Chippenham_United_F.C."])
    # print(id_text["Chippenham_United_F.C."])
    with open(wiki_train_data, 'w', encoding='utf-8') as fin:
    ...
    total_men_count = 0
    with open(wiki_train_data, 'r', encoding='utf-8') as fr:
        for line in fr:
...
def construct_doc_mentions():
    """构造ace2004的输入数据dict"""
    train_path = './clueweb/clueweb_train_xls.txt'
    ...
    print("total_men_count: ", total_men_count)
    train_text_save_path = './clueweb/clueweb_id_text.pkl'
    train_doc_save_path = './clueweb/clueweb_doc_mentions.pkl'
    train_doc_list_path = './clueweb/clueweb_doc_list.pkl'
...
def construct_kb_id_text():
    ...
    men_id_candidates = pd.read_pickle('./clueweb/men_id_candidates_10.pkl')
...
def check_all_candidates_in_prior():
    men_id_candidates = pd.read_pickle('./clueweb/men_id_candidates_10.pkl')
    men2cand_prior = pd.read_pickle('./clueweb/men2cand_prior.pkl')
    for key, value in men_id_candidates.items():
        for id in value:
...
def add_entity_embedding():
    ...
    pd.to_pickle(new_men_id_candidates, './clueweb/men_id_candidates_10_entity.pkl')
    # 保存entity的候选实体
...
def check_gold_ids_in_candidates():
    msnbc_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-clueweb.csv"
    men_id_candidates = pd.read_pickle('./clueweb/men_id_candidates_10_entity.pkl')
    enwiki_id2title = pd.read_pickle("./data/enwiki_id2title2.pkl")
    enwiki_title2id = {v: k for k, v in enwiki_id2title.items()}
    gold_entities_dict = dict()
    unk_gold_id_count = 0
    ...
    print("unk_gold_count: ", unk_gold_count)
    print("unk_gold_count2: ", unk_gold_count2)
    pd.to_pickle(men_id_candidates_copy, './clueweb/men_id_candidates_10_entity.pkl')

# 10. 从doc_mentions里去掉不在men_id_candidates_10_entity里的mention
def revise_doc_mentions():
    ...
    print("unk_count2: ", unk_count2)
    pd.to_pickle(train_doc_mentions_copy, "./clueweb/clueweb_doc_mentions_entity.pkl")

def check_all_candidates_in_embedding():
    entity_embedding = pd.read_pickle('./entity_embedding/albert/entity_embedding_dict_new_entire.pkl')
    men_id_candidates = pd.read_pickle('./clueweb/men_id_candidates_10_entity.pkl')
    for key, value in men_id_candidates.items():
        for id in value:
...
if __name__ == '__main__':
    ...
    # check_gold_ids_in_candidates()
    revise_doc_mentions()
    # check_all_candidates_in_embedding()
