...
def handle_candidate_excel():
    ...
    msnbc_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-msnbc.csv"
    total_cand_cout = 0
    id_null_count = 0
    men_id_candidates = dict()
    men_name_candidates = dict()
    ...
    pd.to_pickle(men_id_candidates, "./msnbc/men_id_candidates.pkl")
    # pd.to_pickle(men_name_candidates, './ace2004/men_name_candidates2.pkl')
    pd.to_pickle(men2cand_prior, './msnbc/men2cand_prior.pkl')
    ...
    pd.to_pickle(id2cand, './msnbc/id2cand.pkl')
...
def filter_candidates():
    ...
    aquaint_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-msnbc.csv"
    men_id_candidates = pd.read_pickle('./msnbc/men_id_candidates.pkl')
    enwiki_id2title = pd.read_pickle("./data/enwiki_id2title2.pkl")
    enwiki_title2id = {v: k for k, v in enwiki_id2title.items()}
    ...
    pd.to_pickle(men_id_candidates_10, './msnbc/men_id_candidates_10.pkl')
...
def construct_aquaint_id_text():
    path = 'D:/DataSet/Entity Linking/WNED/WNED/wned-datasets/msnbc/RawText'
...
def mapping_xls_xml():
    ...
    aida_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-msnbc.csv"
    gt_count = 0
    ...
    aquaint_kb_xml = 'D:/DataSet/Entity Linking/WNED/WNED/wned-datasets/msnbc/msnbc.xml'
...
def ace2004_train_data_xls(xls_offset_dict, id_text):
    ...
    enwiki_id2title = pd.read_pickle("./data/enwiki_id2title2.pkl")
    enwiki_title2id = {v: k for k, v in enwiki_id2title.items()}
    aquaint_train_data = './msnbc/msnbc_train_xls_entity.txt'
    men_id_candidates_10 = pd.read_pickle('./msnbc/men_id_candidates_10_entity.pkl')
    out_count = 0
    out_count2 = 0
...
def construct_doc_mentions():
    """构造ace2004的输入数据dict"""
    train_path = './msnbc/msnbc_train_xls_entity.txt'
    ...
    print("total_men_count: ", total_men_count)
    train_text_save_path = './msnbc/msnbc_id_text_entity.pkl'
    train_doc_save_path = './msnbc/msnbc_doc_mentions_entity.pkl'
    train_doc_list_path = './msnbc/msnbc_doc_list_entity.pkl'
...
def construct_kb_id_text():
    ...
    men_id_candidates = pd.read_pickle('./msnbc/men_id_candidates_10.pkl')
...
def check_all_candidates_in_prior():
    men_id_candidates = pd.read_pickle('./msnbc/men_id_candidates_10.pkl')
    men2cand_prior = pd.read_pickle('./msnbc/men2cand_prior.pkl')
    for key, value in men_id_candidates.items():
        for id in value:
...
def add_entity_embedding():
    ...
    pd.to_pickle(new_men_id_candidates, './msnbc/men_id_candidates_10_entity.pkl')
    # 保存entity的候选实体
...
def check_gold_ids_in_candidates():
    msnbc_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-msnbc.csv"
    men_id_candidates = pd.read_pickle('./msnbc/men_id_candidates_10_entity.pkl')
    enwiki_id2title = pd.read_pickle("./data/enwiki_id2title2.pkl")
    enwiki_title2id = {v: k for k, v in enwiki_id2title.items()}
    gold_entities_dict = dict()
    unk_gold_id_count = 0
