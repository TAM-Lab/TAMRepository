...
def build_radical_pretrain_embedding(embedding_path, word_alphabet, embedd_dim=50, norm=True):
    r = radical.Radical()
    # embedd_dict:  token对应的embedding�?
    embedd_dict = dict()
    if embedding_path != None:
        embedd_dict, embedd_dim = load_pretrain_emb(embedding_path)
    scale = np.sqrt(3.0 / embedd_dim)
    ...
    for word, index in word_alphabet.iteritems():
        # 如果word为中�?
        if u"\u4e00" <= word <= u"\u9fff":
            rad = r.get_radical(word)
            if rad in embedd_dict:
                if norm:
                    # 归一�?
                    pretrain_emb[index, :] = norm2one(embedd_dict[rad])
                else:
                    pretrain_emb[index, :] = embedd_dict[rad]
                perfect_match += 1
            else:
                pretrain_emb[index, :] = np.random.uniform(-scale, scale, [1, embedd_dim])
                not_match += 1
    ...
    print("Embedding:\n     pretrain word:%s, prefect match:%s, oov:%s, oov%%:%s" % (
        pretrained_size, perfect_match, not_match, (not_match + 0.) / word_alphabet.size()))
    return pretrain_emb, embedd_dim
