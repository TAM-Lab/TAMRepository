...
def add_unk_gold_id_in_enwiki_and_kb_id():
    ...
    wiki_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-wikipedia.csv"
    total_cand_cout = 0
    id_null_count = 0
...
def handle_candidate_excel():
    ...
    # pd.to_pickle(men_id_candidates, "./wikipedia/men_id_candidates.pkl")
    # pd.to_pickle(men_name_candidates, './ace2004/men_name_candidates2.pkl')
    # pd.to_pickle(men2cand_prior, './wikipedia/men2cand_prior.pkl')
    ...
    # pd.to_pickle(id2cand, './wikipedia/id2cand.pkl')
...
def filter_candidates():
    ...
    wiki_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-wikipedia.csv"
    men_id_candidates = pd.read_pickle('./wikipedia/men_id_candidates.pkl')
    enwiki_id2title = pd.read_pickle("./data/enwiki_id2title2.pkl")
    enwiki_title2id = {v: k for k, v in enwiki_id2title.items()}
    ...
    # pd.to_pickle(men_id_candidates_10, './wikipedia/men_id_candidates_10.pkl')
...
def construct_aquaint_id_text():
    path = 'D:/DataSet/Entity Linking/WNED/WNED/wned-datasets/wikipedia/RawText'
...
def mapping_xls_xml():
    ...
    aida_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-wikipedia.csv"
    gt_count = 0
    ...
    wiki_kb_xml = 'D:/DataSet/Entity Linking/WNED/WNED/wned-datasets/wikipedia/wikipedia.xml'
    kb_file = open(wiki_kb_xml, 'r', encoding='utf-8')
...
def ace2004_train_data_xls(xls_offset_dict, id_text):
    ...
    enwiki_id2title = pd.read_pickle("./data/enwiki_id2title2.pkl")
    enwiki_title2id = {v: k for k, v in enwiki_id2title.items()}
    wiki_train_data = './wikipedia/wikipedia_train_xls.txt'
    men_id_candidates_10 = pd.read_pickle('./wikipedia/men_id_candidates_10.pkl')
    out_count = 0
    out_count2 = 0
...
def construct_doc_mentions():
    """构造ace2004的输入数据dict"""
    train_path = './wikipedia/wikipedia_train_xls.txt'
    ...
    print("total_men_count: ", total_men_count)
    train_text_save_path = './wikipedia/wikipedia_id_text.pkl'
    train_doc_save_path = './wikipedia/wikipedia_doc_mentions.pkl'
    train_doc_list_path = './wikipedia/wikipedia_doc_list.pkl'
...
def construct_kb_id_text():
    ...
    men_id_candidates = pd.read_pickle('./wikipedia/men_id_candidates_10.pkl')
...
def check_all_candidates_in_prior():
    men_id_candidates = pd.read_pickle('./wikipedia/men_id_candidates_10.pkl')
    men2cand_prior = pd.read_pickle('./wikipedia/men2cand_prior.pkl')
    for key, value in men_id_candidates.items():
        for id in value:
...
def add_entity_embedding():
    ...
    pd.to_pickle(new_men_id_candidates, './wikipedia/men_id_candidates_10_entity.pkl')
    # 保存entity的候选实体
...
def check_gold_ids_in_candidates():
    msnbc_train_path = "D:/DataSet/Entity Linking/data/data/generated/test_train_data/wned-wikipedia.csv"
    men_id_candidates = pd.read_pickle('./wikipedia/men_id_candidates_10_entity.pkl')
    enwiki_id2title = pd.read_pickle("./data/enwiki_id2title2.pkl")
    enwiki_title2id = {v: k for k, v in enwiki_id2title.items()}
    gold_entities_dict = dict()
    unk_gold_id_count = 0
    ...
    pd.to_pickle(men_id_candidates_copy, './wikipedia/men_id_candidates_10_entity.pkl')
...
def revise_doc_mentions():
    ...
    pd.to_pickle(train_doc_mentions_copy, "./wikipedia/wikipedia_doc_mentions_entity.pkl")
