...
def get_loader(root, surface, windows, doc_num, body_num, val, test, shuffle, num_workers, tokenizer, dataset, index):
    ...
    new_mentions, gold_entities_ind, text_id, new_mentions_surface, new_mentions_context, new_document_vec, new_entities_Tvec, new_entities_Bvec, new_mentions2entities, new_entities2entities, sparse_features, new_total_entities, new_m2c_prior, new_entity_sr, mentions2entities, new_context, hands_feature, new_bert_doc_vec = data.getitem(index=index)
    m = len(new_mentions)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--local_model_loc', default='./model_save/aquaint_combine_att_entity/0.901.pkl')
    ...
    for i_index in range(20):
        ...
        title_vec, body_vec, filename, sfeats, total_entities, m2c_prior, entity_sr, mentions2entity, new_context, hand_features, bert_doc_vec = get_loader(
            './data/', 4, 50, 100, 200, val=True, test=False, shuffle=True, num_workers=0, tokenizer=tokenizer, dataset="aquaint", index = i_index)
        ...
        for i, y_lindex in enumerate(y_label_index):
            if y_lindex != local_true_ind[i]:
                print("wrong mention: ", mention[i])
                print("gold entity: ", subject_entities[y_lindex])
                print("wrong entity: ", subject_entities[local_true_ind[i]])
                print("*****---------------------------------*****")
