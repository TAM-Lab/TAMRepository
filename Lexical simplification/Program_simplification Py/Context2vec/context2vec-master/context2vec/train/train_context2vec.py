...
def parse_arguments():
    ...
    # parser.add_argument('--indir', '-i',default="E:\\Simplify\\_Datasets\\train.txt.DIR",help='input corpus directory')
    parser.add_argument('--indir', '-i', default="E:\Simplify\context2vec/train.txt.DIR",help='input corpus directory')
    parser.add_argument('--trimfreq', '-t', default=1, type=int,help='minimum frequency for word in training')
    parser.add_argument('--ns_power', '-p', default=0.75, type=float,help='negative sampling power')
    parser.add_argument('--dropout', '-o', default=0.0, type=float,help='NN dropout')
    parser.add_argument('--wordsfile', '-w',default="E:\Simplify\context2vec\out\EMMBEDDING.targets",help='word embeddings output filename')
    parser.add_argument('--modelfile', '-m',default="E:\Simplify\context2vec\out\MODEL",help='model output filename')
    parser.add_argument('--cgfile', '-cg',default="E:\Simplify\context2vec\out",help='computational graph output filename (for debug)')
    parser.add_argument('--gpu', '-g', default=0, type=int,help='GPU ID (negative value indicates CPU)')
    parser.add_argument('--unit', '-u', default=300, type=int,help='number of units (dimensions) of one context word')
    parser.add_argument('--batchsize', '-b', type=int, default=1,help='learning minibatch size')
    parser.add_argument('--epoch', '-e', default=5, type=int,help='number of epochs to learn')
    parser.add_argument('--context', '-c', choices=['lstm'],default='lstm',help='context type ("lstm")')
    parser.add_argument('--deep', '-d', choices=['yes', 'no'],default="yes",help='use deep NN architecture')
    parser.add_argument('--alpha', '-a', default=0.001, type=float,help='alpha param for Adam, controls the learning rate')
    parser.add_argument('--grad-clip', '-gc', default=12, type=float,help='if specified, clip l2 of the gradient to this value')
